...
#include "llvm/Transforms/Obfuscation/XorStringEncryption.h"

#define DEBUG_TYPE "xorstringencryption"

using namespace llvm;

XorStringEncryption::XorStringEncryption(uint32_t KeySize) : StringEncrypt(ID){
    _key = generateRandomKey(KeySize);
}

XorStringEncryption::XorStringEncryption(const std::string& Key) : StringEncrypt(ID){
    _key = Key;
}
